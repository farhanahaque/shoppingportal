		...
		if($count > 0){
			...
			if (mysqli_query($con,$sql)) {
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Your product is added to cart successfully..!</b>
					</div>
				";
				exit();
			}
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				echo '
					<div class="row">
						...
						<div class="col-md-3"><img class="img-responsive" src="product_images/'.$product_image.'" 
						draggable="false"/></div>
						<div class="col-md-3">'.$product_title.'</div>
						<div class="col-md-3">Tk'.$product_price.'</div>
					</div>';
				...
				while ($row=mysqli_fetch_array($query)) {
					...
					echo 
						'<div class="row">
								...
								<div class="col-md-2"><img class="img-responsive" src="product_images/'.$product_image.'" draggable="false" ></div>
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="payment_success.php" method="GET">
						    <input type="hidden" name="tx" value="'.$_SESSION["uid"].'">
							<input type="hidden" name="st" value="Completed">
					<input type="hidden" name="amt" class="form-control total" value="'.$product_price.'" >
							
							<input type="hidden" name="cc" value="'.$_SESSION["uid"].'">
							<input type="hidden" name="cm" value="'.$_SESSION["uid"].'">
							<input type="hidden" name="ta" class="form-control total" value="'.$product_price.'" >';
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/simpliciaShop/payment_success.php"/>
									...
									<input style="float:right;margin-right:80px;" type="image" name="submit"
										src="product_images/checkout.png" alt="Checkout"
										alt=" The safer, easier way to pay online">
								</form>';
				}
			}
	}
